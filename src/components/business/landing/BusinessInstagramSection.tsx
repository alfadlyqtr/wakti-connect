...
// Fallback component when Instagram embed fails
const InstagramFallback = ({ url, title, description }: { url: string; title: string; description: string }) => (
  <div className="border rounded-lg p-6 text-center max-w-xl mx-auto">
    <img
...
const BusinessInstagramSection: React.FC<BusinessInstagramSectionProps> = ({ content }) => {
  ...
  return (
    <section className="py-12 animate-fade-in">
      ...
      <div className={centeredContent ? "flex justify-center" : ""}>
        <div style={{ maxWidth: `${maxWidth}px` }} className="w-full">
          {embedError ? (
            <InstagramFallback url={instagramUrl} title={title} description={description} />
          ) : (
            <blockquote
...
// Add window.instgrm type definition to avoid TypeScript errors
declare global {
  interface Window {
    instgrm?: {
      Embeds: {
