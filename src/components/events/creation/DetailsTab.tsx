    ...
    <div className="px-4 py-2 space-y-6 max-w-2xl mx-auto">
      <div className="space-y-3">
        <div>
          ...
          <Input 
            id="title"
            placeholder={t("events.enterEventDetails")}
          ...
          {title.trim() === '' && (
            <p className="text-sm text-amber-600 mt-1">{t("validation.required")}</p>
          )}
        </div>
        
        <div>
          <Label htmlFor="description" className="text-base">{t("events.eventDescription")}</Label>
          <Textarea 
            id="description"
      ...
      <div>
        <Label className="text-base">{t("events.dateTime")}</Label>
        <div className="mt-2 space-y-4">
          <DatePicker 
            date={selectedDate} 
            setDate={setSelectedDate}
            placeholder={isMobile ? t("events.selectDate") : t("events.selectEventDate")}
          />
      ...
      <div className="pt-4 flex justify-end">
        ...
        >
          {t("common.next")}: {t("events.customize")}
        </Button>
      </div>
    </div>
  );
